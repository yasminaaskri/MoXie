    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          {selectedEvent && selectedEvent.transcript && (
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const file = new Blob([selectedEvent.transcript], { type: 'text/plain' });
                    element.href = URL.createObjectURL(file);
                    element.download = `Transcript-${selectedEvent.title.replace(/\s+/g, '-')}.txt`;
                    document.body.appendChild(element);
                    element.click();
                    document.body.removeChild(element);
                  }}
                  className="px-3 py-1 bg-green-600 text-white text-xs rounded hover:bg-green-700 flex items-center gap-1 transition-colors shadow-sm"
                >
                  <FiUpload className="w-3 h-3" /> Télécharger
                </button>
              </div>
              <div className="relative">
                <textarea
                  readOnly
                  value={selectedEvent.transcript}
                  rows="4"
                  className="w-full p-3 border border-green-300 rounded-lg focus:ring-2 focus:ring-green-500 bg-white text-xs font-mono text-gray-700 resize-y"
                />
              </div>
            </div>
          )}
